...
async function applyRazorpayFix() {
  const sqlPath = path.join(__dirname, '..', 'supabase', 'fix_razorpay_tables.sql');
  ...
  console.log("Applying Razorpay DB Fix via temp-sql...");
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      console.error("❌ Failed to apply SQL. Status:", res.status);
    }
  } catch (err) {
